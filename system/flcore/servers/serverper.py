...
class FedPer(Server):
    ...
    def send_models(self):
        ...
        for client in self.selected_clients:
            ...
            # Chỉ gửi phần base của global model
    ...
    def aggregate_parameters(self):
        ...
        # Xóa các trọng số cũ của phần base trên server
        for param in self.global_model.base.parameters():
            param.data.zero_()
        
        # Tổng hợp các base model nhận được từ client vào self.global_model.base
        for w, client_base_model in zip(self.uploaded_weights, self.uploaded_models):
            for server_param, client_param in zip(self.global_model.base.parameters(), client_base_model.parameters()):
